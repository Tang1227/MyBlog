...
public class TagController {
    ...
    @GetMapping("/tags")
    public String tags(@PageableDefault(size = 4,sort = {"id"},
    ...
    @PostMapping("/tags")
    public String saveTags(@Valid Tag tag, BindingResult result, RedirectAttributes redirectAttributes){
        //重复验证
        if(tagService.findTagByName(tag.getName())!=null){
            result.rejectValue("name","nameError","不可以重复添加标签");
        }
        if(result.hasErrors()){
            return "admin/tags-input";
        }
